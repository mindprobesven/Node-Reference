...
function processStrand({
  ...
  strand.map((gene, index) => {
    let candidates = []
    for (let chars = minSize; chars <= maxSize; chars += 1) {
      const candidate = d.substring(index, index + chars)
      candidates = (genesLookup[candidate] && candidate.length === chars) ? [...candidates, candidate] : candidates
    }
